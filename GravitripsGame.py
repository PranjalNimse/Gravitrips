...
class GravitripsGame:
    ...
    # method to set the count of all placed pieces on the gameboard
    def checkPieceCount(self):
        self.piece_count = sum(1 for row in self.gameboard for piece in row if piece)

    # method to get the count of all placed pieces on the gameboard
    def getPieceCount(self):
        return sum(1 for row in self.gameboard for piece in row if piece)

    # method to display the gameboard with placed pieces and empty places
    ...
    # Minimax algorithm implementation for making moves.
    ...
    # the cut-off function that decides when it is appropriate to cut off the search in alpha beta pruning
    ...
    # method to change the current move
    ...
    # method to check which player is the winner at given moment
    def check_winner(self):
        if self.player2Score == 1:
            self.win_value = 2
        elif self.player1Score == 1:
            self.win_value = 1
